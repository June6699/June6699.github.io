...
结果真正折腾起来，发现这件事最麻烦的不是安装，也不是 Node 版本，而是判断“好了”到底是什么意思。
...
这里能返回 `OK`，再谈 QQ bot 能不能用。否则 channel 在线只是在线，不是可用。

QQ 的上下文也顺手补了一下。OpenClaw 里显式设了群聊历史窗口：
...
`WECHAT_CONTEXT_TURNS='8'`，大概保留最近几轮。它不是长期记忆，进程重启就没了，但日常聊天够用。为了防止上下文串味，又加了 `/清空上下文`。

## 五、模型接口要在服务器上测

今天试过好几个中转接口。最容易出错的判断是：我本机能用，所以服务器也能用。

实际不是。Windows 上 Codex、Claude Code 能跑，不代表 `june-server` 出口到那个站也稳。有的接口在服务器上 TLS 握手超时，有的能连上但上游返回：
...
`deepseek-chat` 也能用，但实际映射到 `deepseek-v4-flash`。`deepseek-reasoner` 有一次返回空 content，不适合直接拿来当群聊 bot。最后选 `deepseek-v4-flash`，主要是因为群聊更看重响应速度和稳定性，不是每句都要深度推理。
...
```env
IMAGE_API_URL='https://api.vibelearning.top/v1/images/generations'
...
这不是完美安全方案，只是第一道门。对群 bot 来说，这道门必须有。因为总会有人随手发一些边界词测试，bot 不应该把这些东西继续转给上游。
...
不过登录成功也不是故事结束。后面还会看到 `1205 == 0`、`-1 == 0`、`AggregateError` 这些 wechat4u 的同步警告。它们不一定马上让 bot 死掉，但说明这条 Web WeChat 链路并不稳。最后还是要在群里真的 @ 一句，看它能不能收到并回复。

## 九、最后留下的一组检查命令

今天最后留下来的不是“完美方案”，而是一组我以后会先跑的检查。
...
这些命令看着有点啰嗦，但比凭感觉判断可靠。尤其是这种聊天机器人，坏的时候经常不是全坏，而是某一段坏了。

## 十、收尾

今天最有用的经验，其实是一句话：别急着说好了。
...
机器人看起来像一个小玩具，真跑起来更像一串管道。每段都可能漏水。以后再修它，第一件事不是重启，而是先问：现在坏的是哪一层。
